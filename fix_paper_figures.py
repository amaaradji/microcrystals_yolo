...
# ── 1. metrics_yolo11m.png (first 100 epochs only) ───────────────────────────
def regenerate_metrics():
    ...
    ax = axes[0]
    ax.plot(epochs, tbox, label="Train box loss")
    ax.plot(epochs, tcls, label="Train cls loss")
    ax.plot(epochs, vbox, label="Val box loss", linestyle="--")
    ax.plot(epochs, vcls, label="Val cls loss",  linestyle="--")
    ax.set_xlabel("Epoch"); ax.set_ylabel("Loss")
    ax.set_title("(a)  Training & Validation Loss", fontweight="bold")
    ax.legend(fontsize=9); ax.grid(True, alpha=0.3)

    ax = axes[1]
    ax.plot(epochs, prec,  label="Precision")
    ax.plot(epochs, rec,   label="Recall")
